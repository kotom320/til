...
- **노드 기반 문서 모델**: 각 텍스트 블록, 이미지, 리스트가 개별 노드로 관리. 트리 구조로 연결되어 문서 전체 구성
- **불변성 기반 상태 관리**: 상태 변경 시 새로운 상태 객체 생성. 변경 추적과 최적화에 유리한 구조

## 5. 명령어(Commands) 및 이벤트 시스템

- **Commands**: `editor.registerCommand()`으로 포맷팅, 삽입, 삭제 명령 정의. `COMMAND_PRIORITY_*`로 우선순위 제어
- **이벤트 브로드캐스팅**: 상태 변경, 사용자 입력 등 다양한 이벤트를 구독(subscribe). 플러그인 또는 외부 컴포넌트가 반응 가능
